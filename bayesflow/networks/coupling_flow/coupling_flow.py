...
class DoubleInferenceNetwork(keras.Layer):
    def __init__(self, base_distribution: InferenceNetwork, **kwargs):
        super().__init__(**layer_kwargs(kwargs))
        self.base_distribution = base_distribution
        self.base_distribution.trainable = False
    ...
    def build(self, xz_shape: Shape, conditions_shape: Shape = None) -> None:
        ...
        if not self.base_distribution.built:
            self.base_distribution.build(xz_shape, conditions_shape)
    ...
    def sample(self, batch_shape: Shape, conditions: Tensor = None, **kwargs) -> Tensor:
        samples = self.base_distribution.sample(batch_shape, conditions, **kwargs)
...
@serializable("bayesflow.networks")
class DoubleCouplingFlow(DoubleInferenceNetwork):

    def __init__(
        self,
        base_distribution: InferenceNetwork,
        subnet: str | type = "mlp",
        depth: int = 6,
        transform: str = "affine",
        permutation: str | None = "random",
    ...
    def _forward(
        ...
        if density:
            log_density_latent = self.base_distribution.log_prob(z, conditions, training=False, **kwargs)
            log_density = log_density_latent + log_det
            return z, log_density
    ...
    def _inverse(
        ...
        if density:
            log_prob = self.base_distribution.log_prob(z, conditions, training=False, **kwargs)
            log_density = log_prob - log_det
            return x, log_density
